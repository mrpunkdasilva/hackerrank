...
2. Apply a WHERE clause with conditions combined with OR:
   - Ensure LAT_N and LONG_W are not NULL
   - Check if the first character of the CITY name is NOT a vowel OR
...
3. Add the WHERE clause with conditions:
   ...
   - OR use SUBSTRING with a negative index to extract the last character and check if it's NOT a vowel
   ```sql
   WHERE 
...
The query will return a list of unique city names from the STATION table where either the first character is NOT a vowel OR the last character is NOT a vowel, sorted alphabetically.